...
// Display names only; stored and API status values stay baru/diproses/selesai/dibatalkan.
const orderStatusLabels={baru:'Pesanan masuk',diproses:'Diproses',selesai:'Selesai',dibatalkan:'Dibatalkan'};
 table('ai-orders',['ID','Pelanggan','Item','Total','Status','Tindakan'],orders,o=>[o.id,o.customer,o.items.map(i=>(i.product_name??i.name)+' × '+i.quantity).join(', '),money(o.total),orderStatusLabels[o.status]??o.status,button('Kelola',()=>{const f=$('ai-order-edit-form');f.elements.id.value=o.id;f.elements.status.value=o.status;f.elements.notes.value=o.notes;$('ai-order-detail').textContent=o.customer+' · '+o.items.map(i=>(i.product_name??i.name)+' × '+i.quantity+' @ '+money(i.price)).join(', ');$('ai-order-edit-dialog').showModal();})]);
...
save.textContent='Simpan model';save.classList.add('ai-legacy-provider');routesHeading.textContent='Model per tingkat';routes.prepend(routesHeading);routes.querySelector('p').textContent='Cukup pilih profil yang sudah dibuat untuk setiap tingkat; API key tetap aman tersimpan di server.';
