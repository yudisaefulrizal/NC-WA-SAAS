...
async function loadFailures(){table('ai-failures',['Waktu','Akun','Sesi','Agent','Error','Pesan pelanggan'],await api('/api/admin/ai/failures'),r=>[new Date(r.created_at).toLocaleString('id-ID'),r.account_id,r.session_id,r.agent||'—',r.error,r.message]);}
$('failures-refresh').onclick=()=>run(loadFailures);
let aiAdjustment;
