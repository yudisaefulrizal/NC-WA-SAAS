...
async function loadAIData(id=$('ai-session').value,generation=assistantLoad){if(!id)return;const base='/sessions/'+encodeURIComponent(id)+'/ai';const [products,orders]=await Promise.all([api(base+'/products'),api(base+'/orders')]);if(generation!==assistantLoad||id!==$('ai-session').value)return;
 table('ai-products',['Nama','Jenis','Deskripsi','Harga','Stok/kapasitas','Status','Tindakan'],products,p=>[p.name,p.type==='service'?'Layanan':'Produk',p.description,money(p.price),p.stock,p.active?'Aktif':'Nonaktif',button('Edit',()=>openProduct(p))]);
...
function openProduct(product){const f=$('ai-product-form');f.reset();editingProductName=product?product.name:'';if(product)for(const key of ['name','type','description','price','stock'])f.elements[key].value=product[key];f.elements.active.checked=product?product.active:true;$('ai-product-dialog').showModal();}
$('ai-product-add').onclick=()=>openProduct();
aiDataForm('ai-product-form',async data=>{const id=$('ai-session').value,payload={...data,price:Number(data.price),stock:Number(data.stock),active:data.active==='on'};if(editingProductName)await api('/sessions/'+encodeURIComponent(id)+'/ai/products/'+encodeURIComponent(editingProductName),'PUT',payload);else await api('/sessions/'+encodeURIComponent(id)+'/ai/products','POST',payload);$('ai-product-dialog').close();await loadAIData();$('message').textContent='Produk tersimpan.';});
let orderRequest;
