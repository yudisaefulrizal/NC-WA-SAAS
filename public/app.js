...
async function loadAIData(id=$('ai-session').value,generation=assistantLoad){if(!id)return;const base='/sessions/'+encodeURIComponent(id)+'/ai';const [products,orders]=await Promise.all([api(base+'/products'),api(base+'/orders')]);if(generation!==assistantLoad||id!==$('ai-session').value)return;
 table('ai-products',['Kode','Nama','Jenis','Harga','Stok/kapasitas','Satuan','Status','Tindakan'],products,p=>[p.id,p.name,p.type==='service'?'Layanan':'Produk',money(p.price),p.stock,p.unit,p.active?'Aktif':'Nonaktif',button('Edit',()=>openProduct(p))]);
 $('ai-product-codes').replaceChildren(...products.filter(p=>p.active).map(p=>new Option(p.name,p.id)));
 table('ai-orders',['ID','Pelanggan','Item','Total','Status','Tindakan'],orders,o=>[o.id,o.customer,o.items.map(i=>i.name+' × '+i.quantity).join(', '),money(o.total),o.status,button('Kelola',()=>{const f=$('ai-order-edit-form');f.elements.id.value=o.id;f.elements.status.value=o.status;f.elements.notes.value=o.notes;$('ai-order-detail').textContent=o.customer+' · '+o.items.map(i=>i.name+' × '+i.quantity+' @ '+money(i.price)).join(', ');$('ai-order-edit-dialog').showModal();})]);
...
function openProduct(product){const f=$('ai-product-form');f.reset();f.elements.id.readOnly=Boolean(product);if(product)for(const key of ['id','name','type','description','price','stock','unit'])f.elements[key].value=product[key];f.elements.active.checked=product?product.active:true;$('ai-product-dialog').showModal();}
...
async function loadAIConfig(){const config=await api('/api/admin/ai');for(const name of ['endpoint','model_cheap','model_medium','model_smart','input_rate','output_rate','memory_limit','context_memory_limit','credit_price'])$('ai-config').elements[name].value=config[name];$('ai-config').elements.apiKey.value='';$('ai-config-status').textContent=config.configured?'API key tersimpan terenkripsi.':'Koneksi AI belum dikonfigurasi.';}
form('ai-config',async data=>{for(const key of ['input_rate','output_rate','memory_limit','context_memory_limit','credit_price'])data[key]=Number(data[key]);await api('/api/admin/ai','PUT',data);await loadAIConfig();$('message').textContent='Pengaturan AI tersimpan. Batas memori berlaku untuk seluruh percakapan.';});
