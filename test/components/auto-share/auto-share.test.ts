...
import {createAutoShare,contactInput,templateInput,jobInput,nextSchedule,randomDelay} from '../../../src/components/auto-share/domain/auto-share.js';
import {decodeHeaders,mediaVariable,parsePlaceholders,renderTemplate,validateHeaders,validateSourceData,validateSourceMedia} from '../../../src/components/auto-share/domain/source.js';
import {acceptTidyResult,defaultTidyPrompt,tidyMessage,tidyMessages,tidyNoteInput} from '../../../src/components/auto-share/domain/tidy.js';
...
app.use((req,res,next)=>{res.locals.accountId=req.get('account');next();});app.use(autoShareRouter(service));app.use((e:Error,_req:express.Request,res:express.Response,_next:express.NextFunction)=>res.status(e instanceof ApiError?e.status:500).json({message:e.message}));
