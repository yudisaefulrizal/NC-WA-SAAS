...
import {runAgents,updateRouterContext,defaultTools,type ToolContext} from '../../../src/components/ai/domain/pipeline/runner.js';
...
test('A specialist can correct invalid order input without repeating a successful mutation',async()=>{
 const {ApiError}=await import('../../../src/libraries/errors.js');let calls=0,mutations=0;
 const result=await runAgents(async(c,m)=>{
...
test('Pesanan runs on the Terstruktur tier with provider schema, and in prompt mode on other tiers',async()=>{
 const {aiRequestPayload}=await import('../../../src/components/ai/domain/provider.js');const {defaultWorkflow}=await import('../../../src/components/ai/domain/pipeline/workflow.js');
