...
import {defaults,chatEndpoint,type AITransport,type AIMessage} from '../../../src/components/ai/domain/provider.js';
...
async function fixture(call:AITransport=async()=> 'Jawaban bisnis',sendFail=false,wait:(ms:number)=>Promise<void>=async()=>{},events:string[]=[],presenceFail=false,raw=false,tools?:AITools){
 ...
 },wait,tools);await service.adjust(id,id,{amount:10000,reason:'fixture',requestId:'fixture'});await service.saveAssistant(id,'shop',{enabled:true,profile:{faq:'Produk tersedia'},behavior:'Gunakan bahasa Indonesia'});
...
test('AI wallet adjustments are idempotent and HTTP owner configuration is private',async()=>{
 ...
 const {createApp}=await import('../../../src/http/app.js');const app=createApp(),token=randomUUID();await db.execute('INSERT INTO login_sessions VALUES (?,?,DATE_ADD(UTC_TIMESTAMP(),INTERVAL 1 HOUR))',[digest(token),f.id]);const cookie='ncwa_session='+token,origin=process.env.APP_ORIGIN??'http://127.0.0.1:8067';
...
test('Gateway assistant routes verify session ownership and account isolation',async()=>{
 const {mkdtemp,rm}=await import('node:fs/promises'),{tmpdir}=await import('node:os'),{join}=await import('node:path');const {createGateway}=await import('../../../src/http/gateway.js');const {createApp}=await import('../../../src/http/app.js');
 ...
 await request(app).put('/sessions/shop/ai').set('Cookie','ncwa_session='+tokens[0]).set('Origin',origin).send({enabled:true,profile:{faq:'Tenant A only'},behavior:'',accountId:g.id}).expect(200);
 const other=await request(app).get('/sessions/shop/ai').set('Cookie','ncwa_session='+tokens[1]).expect(200);assert.equal(other.body.profile.faq,'Produk tersedia');assert.ok(other.body.knowledge.includes('Produk tersedia'));assert.ok(!other.body.knowledge.includes('Tenant A only'));
...
test('Multi-agent WhatsApp flow switches agents, persists shared memory across restart, and isolates identical customer IDs',async()=>{
 ...
 await g.service.saveAssistant(g.id,'shop',{enabled:true,profile:{faq:'Tenant B only'},behavior:'',products_source:{mode:'endpoint',endpoint:'https://8.8.8.8/products'},orders_source:{mode:'builtin'}});
...
test('WhatsApp transaction creates a built-in order, support reads it using shared memory, and Knowledge stays behind its tool',async()=>{
 const {aiData}=await import('../../../src/components/ai/domain/profiles/cs/data.js');let orderId='';
 const transport:AITransport=async(_c,m)=>{
  if(m[0].content.startsWith('Anda adalah Context Agent'))return 'pelanggan-menunggu-pesanan';
...
test('Product photo is sent before the text answer, via WhatsApp media, and only for the final answer',async()=>{
 const {aiData}=await import('../../../src/components/ai/domain/profiles/cs/data.js');
...
test('Owner model configuration persists, legacy fallback works, tests select each tier, and client usage hides models',async()=>{
 ...
 const {createApp}=await import('../../../src/http/app.js');const token=randomUUID();await db.execute('INSERT INTO login_sessions VALUES (?,?,DATE_ADD(UTC_TIMESTAMP(),INTERVAL 1 HOUR))',[digest(token),tiered.id]);
