...
import {AIData,source,sourceInput,endpointUrl,callEndpoint,orderInput,productForAI,type Product} from '../../../src/components/ai/domain/profiles/cs/data.js';
...
test('Authenticated product/order APIs enforce ownership, CSRF and immutable order amounts',async()=>{
 ...
 const {createGateway}=await import('../../../src/http/gateway.js'),{createApp}=await import('../../../src/http/app.js');
...
test('Terstruktur tier: every provider profile and route carries it, runtime uses it, and migration copies it from Murah',async()=>{
 const {migrateAI}=await import('../../../src/components/ai/data-access/schema.js');const {encrypt}=await import('../../../src/libraries/crypto.js');
