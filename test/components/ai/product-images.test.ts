// Tes foto produk: diperkecil, diubah ke JPEG, dan disimpan per akun.
...
import { ProductImageStore } from '../../../src/components/ai/domain/profiles/cs/product-images.js';
...
// Foto milik sebuah data profil; setiap akun uji punya satu.
...
test('Uploaded photo is downscaled to the width limit, re-encoded to JPEG, and stored per account', async () => {
  ...
  const [rows] = await db.execute<any[]>('SELECT account_id,data_profile_id FROM ai_product_images WHERE id=?', [
    saved.id,
  ]);
...
test('A narrower photo is kept at its original width instead of being enlarged', async () => {
  const accountId = await account();
  const small = await sharp({ create: { width: 400, height: 300, channels: 3, background: { r: 10, g: 10, b: 10 } } })
    .png()
    .toBuffer();
...
test("Non-image uploads are rejected and accounts cannot read each other's photos", async () => {
  const a = await account(),
    b = await account();
  await assert.rejects(store.save(a, profileOf(a), 'doc.txt', Readable.from(Buffer.from('not an image'))), {
    code: 'unsupported_file_type',
  });
  const png = await sharp({ create: { width: 10, height: 10, channels: 3, background: { r: 1, g: 2, b: 3 } } })
    .png()
    .toBuffer();
