// Tes parser dan validasi pesanan: format baris tetap, pencocokan katalog, dan perbaikan jawaban AI.
...
import {
  ...
  extractOrderJson,
  catalogName,
} from '../../../src/components/ai/domain/profiles/cs/order-schema.js';
const names = ['Kopi Susu', 'Teh Manis', 'Xiaomi Redmi', 'Roti, Keju', '3 Second Coffee'];
const order = (items: [string, number][], notes = '') => ({
  items: items.map(([product_name, quantity]) => ({ product_name, quantity })),
  notes,
});

test('Parser reads the fixed line format and its common variants', () => {
  assert.deepEqual(
    parseOrderText('2 x Kopi Susu; 1 x Teh Manis; catatan: kurang manis', names),
    order(
      [
        ['Kopi Susu', 2],
        ['Teh Manis', 1],
  ...
  assert.deepEqual(
    parseOrderText('2 Kopi Susu, 1 x Teh Manis, catatan: antar sore; bayar tunai', names),
    order(
      [
        ['Kopi Susu', 2],
        ['Teh Manis', 1],
      ],
      'antar sore; bayar tunai',
    ),
  );
  // Nama yang diawali x, mengandung koma, atau diawali angka tetap dieja persis.
...
test('Parser returns null instead of guessing', () => {
  for (const text of [
    ...
    '2 x Kopi',
    '2 x Es Teh',
    '2 x Kopi Susu; 3 x Kopi Susu',
    '0 x Kopi Susu',
    '3 Second Coffee',
    'catatan: kurang manis',
  ])
    assert.equal(parseOrderText(text, names), null, text);
});
...
test('AI replies are repaired deterministically, then validated against the catalog', () => {
  assert.deepEqual(
    ...
    order([['Kopi Susu', 2]], 'kurang manis'),
  );
  assert.deepEqual(
    validateOrderOutput(
      {
  ...
  for (const value of [
    { lengkap: true, items: [{ product_name: 'Es Teh', quantity: 1 }], notes: '' },
    {
      lengkap: true,
      items: [
        { product_name: 'Kopi Susu', quantity: 1 },
        { product_name: 'Kopi Susu', quantity: 2 },
      ],
      notes: '',
    },
    { lengkap: true, items: [{ product_name: 'Kopi Susu', quantity: 'dua' }], notes: '' },
    { lengkap: true, items: [], notes: '' },
    { lengkap: 'ya', items: [], notes: '' },
    { lengkap: true, items: [{ product_name: 'Kopi Susu', quantity: 1 }], notes: '', extra: 1 },
  ])
    assert.throws(() => validateOrderOutput(value as any, names), /ai_invalid_order/);
  for (const raw of ['tidak ada JSON', '{rusak}', '[1,2]'])
    assert.throws(() => extractOrderJson(raw), /ai_invalid_order/);
});
