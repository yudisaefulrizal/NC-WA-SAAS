// Tes profil CS Lembaga Pendidikan: data profil, program, kontak, dokumen, batasnya, tool AI, pengiriman dokumen,
// dan simulasi AI Studio.
...
const root = await mkdtemp(join(tmpdir(), 'ncwa-edu-'));
const accounts: string[] = [];
// Router mengirim semuanya ke Program; Program mengirim brosur sekali, lalu menjawab. Setiap panggilan disimpan.
...
const transport: AITransport = async (config, messages) => {
  ...
  if (config.call_role === 'router')
    return JSON.stringify({ s_p_o_konteks: 'wali menanyakan program', sub_agent: 'program', isi_pesan: latest });
  ...
  if (results.length)
    return JSON.stringify({
      answer: results.at(-1)!.content.includes('"available":true')
        ? 'Ini brosurnya.'
        : 'Brosurnya sudah dikirim di atas.',
    });
...
const updates = new Map<string, (event: Update) => void>(),
  sent: { account: string; content: Outbound; file?: Buffer }[] = [];
let sequence = 0;
const gateway = createGateway(
  account => async (session, update) => {
    ...
    return {
      ...
      async typing() {},
      async read() {},
      async send(_jid: string, content: Outbound) {
        const id = 'OUT' + ++sequence;
        sent.push({ account, content, file: 'url' in content ? await readFile(content.url) : undefined });
...
app.use((e: Error, _q: express.Request, r: express.Response, _n: express.NextFunction) =>
  r
    .status(e instanceof ApiError ? e.status : ((e as { status?: number }).status ?? 500))
...
async function tenant(sessions = ['psb']) {
  ...
  const send = (session: string, messageId: string, text: string) =>
...
const upload = (
  ...
  t
    ...
    .set('X-Description', encodeURIComponent(description))
    .send(file);
const files = async (account: string) =>
  (await readdir(join(root, 'files', 'ai-documents', account)).catch(() => [] as string[])).filter(
    f => !f.endsWith('.part'),
  );
...
test('CS Lembaga Pendidikan starts switched off; once on, clients create data profiles with the institution kind', async () => {
  ...
  assert.deepEqual(
    (await clientProfiles(t.id)).map(p => p.id),
    ['cs'],
  );
  ...
  const created = (
    await t.api('post', '/ai/data-profiles').send({ profile_type: 'pendidikan', name: "Ma'had Darul Ilmi" }).expect(201)
  ).body;
  assert.deepEqual(
    [created.profile_type, created.knowledge, created.edu],
    ['pendidikan', '', { lembaga: '', jadwal: '' }],
  );
  // Bidang teks: setiap profil hanya menerima bidangnya; FAQ lembaga lebih panjang daripada FAQ CS.
  const base = '/ai/data-profiles/' + created.id;
  await t
    .api('patch', base + '/field')
    .send({ field: 'edu_lembaga', value: 'Pesantren di Lembang.' })
    .expect(200);
  await t
    .api('patch', base + '/field')
    .send({ field: 'edu_jadwal', value: 'J'.repeat(8000) })
    .expect(200);
  await t
    .api('patch', base + '/field')
    .send({ field: 'edu_jadwal', value: 'J'.repeat(8001) })
    .expect(400);
  await t
    .api('patch', base + '/field')
    .send({ field: 'faq', value: 'F'.repeat(4000) })
    .expect(200);
  await t
    .api('patch', base + '/field')
    .send({ field: 'edu_kind', value: 'kampus' })
    .expect(400);
  await t
    .api('patch', base + '/field')
    .send({ field: 'edu_peserta', value: 'mahasantri' })
    .expect(400);
  const updated = (
    await t
      .api('patch', base + '/field')
      .send({ field: 'behavior', value: 'Sebut peserta didik sebagai mahasantri.' })
      .expect(200)
  ).body;
  assert.deepEqual(
    [updated.behavior, updated.edu.lembaga, updated.profile.faq.length],
    ['Sebut peserta didik sebagai mahasantri.', 'Pesantren di Lembang.', 4000],
  );
  await t
    .api('patch', base + '/field')
    .send({ field: 'usaha', value: 'Toko' })
    .expect(400);
  await t
    .api('patch', base + '/field')
    .send({ field: 'products_source', value: { mode: 'builtin' } })
    .expect(400);
  const cs = (await t.api('post', '/ai/data-profiles').send({ profile_type: 'cs', name: 'Kantin' }).expect(201)).body;
  await t
    .api('patch', '/ai/data-profiles/' + cs.id + '/field')
    .send({ field: 'edu_jadwal', value: 'x' })
    .expect(400);
  await t
    .api('patch', '/ai/data-profiles/' + cs.id + '/field')
    .send({ field: 'faq', value: 'F'.repeat(2001) })
    .expect(400);
  // Produk dan pesanan tetap data CS; data pendidikan tetap data pendidikan.
  await t
    .api('post', base + '/products')
    .send({ name: 'Buku', type: 'product', description: '', price: 1, stock: 1, active: true })
    .expect(409);
  await t
    .api('post', '/ai/data-profiles/' + cs.id + '/programs')
    .send({ name: 'Tahfidz', description: '' })
    .expect(409);
  ...
  assert.deepEqual(
    [summary.profile_name, summary.programs, summary.documents, summary.contacts, summary.sessions],
    ['CS Lembaga Pendidikan', 0, 0, 0, ['psb']],
  );
});

test('Programs, contacts and documents are kept per data profile with limits, and never leak across accounts', async () => {
  const t = await tenant(),
    other = await tenant(['lain']);
  const profile = (
      await t.api('post', '/ai/data-profiles').send({ profile_type: 'pendidikan', name: 'SMA Uji' }).expect(201)
    ).body,
    base = '/ai/data-profiles/' + profile.id;
  const program = (
    await t
      .api('post', base + '/programs')
      .send({ name: 'IPA', description: 'Biaya Rp 500.000 per bulan.' })
      .expect(201)
  ).body;
  await t
    .api('post', base + '/programs')
    .send({ name: 'ipa', description: '' })
    .expect(409);
  await t
    .api('post', base + '/programs')
  ...
  await t
    .api('post', base + '/programs')
    .send({ name: 'IPS', description: 'D'.repeat(4001) })
    .expect(400);
  await t
    .api('put', base + '/programs/' + program.id)
    .send({ name: 'IPA Unggulan', description: 'Biaya Rp 600.000.' })
    .expect(200);
  await t
    .api('post', base + '/programs')
    .send({ name: 'IPS', description: '' })
    .expect(201);
  assert.deepEqual(
    (await t.api('get', base + '/programs').expect(200)).body.map((p: any) => p.name),
    ['IPA Unggulan', 'IPS'],
  );
  ...
  const contact = (
    await t
      .api('post', base + '/contacts')
      .send({ bagian: 'Keuangan', kontak: '0812-7000-5566', deskripsi: 'Pembayaran SPP' })
      .expect(201)
  ).body;
  await t
    .api('post', base + '/contacts')
    .send({ bagian: 'Pendaftaran', kontak: '', deskripsi: '' })
    .expect(400);
  await t
    .api('put', base + '/contacts/' + contact.id)
    .send({ bagian: 'Keuangan', kontak: '0812-7000-0000', deskripsi: 'SPP dan keringanan biaya' })
    .expect(200);
  assert.deepEqual(
    (await t.api('get', base + '/contacts').expect(200)).body.map((c: any) => [c.bagian, c.kontak]),
    [['Keuangan', '0812-7000-0000']],
  );
  // Dokumen: jenis asli dari isi file, gambar sampai 5 MB, file lain sampai 10 MB, deskripsi wajib.
  const brochure = (await upload(t, base + '/documents', pdf, 'brosur.pdf', 'Brosur umum').expect(201)).body;
  assert.deepEqual(
    [brochure.media_type, brochure.mimetype, brochure.filename],
    ['document', 'application/pdf', 'brosur.pdf'],
  );
  ...
  assert.equal(
    (await upload(t, base + '/documents', zip, 'formulir.docx', 'Formulir').expect(201)).body.mimetype,
  ...
  // Ganti file mempertahankan dokumen dan deskripsinya; file lama dihapus.
  const replaced = (
    await upload(t, base + '/documents/' + brochure.id + '/file', png, 'brosur-baru.png', '', 'put').expect(200)
  ).body;
  assert.deepEqual([replaced.id, replaced.filename, replaced.media_type], [brochure.id, 'brosur-baru.png', 'image']);
  assert.equal((await files(t.id)).length, 3);
  await t
    .api('patch', base + '/documents/' + brochure.id)
    .send({ description: 'Brosur PSB 2027' })
    .expect(200);
  const download = await t.api('get', base + '/documents/' + brochure.id + '/file').expect(200);
  assert.deepEqual(
    [download.headers['content-type'], download.headers['content-disposition']],
    ['image/png', "inline; filename*=UTF-8''brosur-baru.png"],
  );
  await other.api('get', base + '/documents/' + brochure.id + '/file').expect(404);
  const listed = (await t.api('get', base + '/documents').expect(200)).body;
  assert.deepEqual(
    listed.map((d: any) => [d.filename, d.description]),
    [
      ['brosur-baru.png', 'Brosur PSB 2027'],
      ['denah.png', 'Denah lokasi'],
      ['formulir.docx', 'Formulir'],
    ],
  );
  // Data yang sama bisa diakses lewat sesi yang menjalankan data profil ini.
  ...
  // Menggandakan menyalin setiap file terpisah; menghapus salah satu data profil tetap menyimpan file yang lain.
  const copy = (
    await t.api('post', '/ai/data-profiles').send({ name: 'Salinan SMA', copy_from: profile.id }).expect(201)
  ).body;
  assert.deepEqual(
    [
      (await t.api('get', '/ai/data-profiles/' + copy.id + '/programs').expect(200)).body.length,
      (await t.api('get', '/ai/data-profiles/' + copy.id + '/contacts').expect(200)).body.length,
      (await t.api('get', '/ai/data-profiles/' + copy.id + '/documents').expect(200)).body.length,
    ],
    [2, 1, 3],
  );
  ...
  await t.api('delete', base + '/programs/' + program.id).expect(200);
  await t.api('delete', base + '/contacts/' + contact.id).expect(200);
  // Maksimal 20 dokumen dan 100 MB per data profil; unggahan yang ditolak tidak meninggalkan file.
  const limited = (
      await t.api('post', '/ai/data-profiles').send({ profile_type: 'pendidikan', name: 'Batas Dokumen' }).expect(201)
    ).body,
    limitBase = '/ai/data-profiles/' + limited.id,
    before = (await files(t.id)).length;
  await db.execute(
    'INSERT INTO ai_edu_documents(id,account_id,data_profile_id,filename,mimetype,media_type,size_bytes,description,position) VALUES (?,?,?,?,?,?,?,?,1)',
    [randomUUID(), t.id, limited.id, 'besar.pdf', 'application/pdf', 'document', 100 * 1024 * 1024 - 10, 'Isian kuota'],
  );
  ...
  for (let i = 0; i < 20; i++)
    await db.execute(
      ...
      [randomUUID(), t.id, limited.id, 'd' + i + '.pdf', 'application/pdf', 'document', 1, 'Isian', i],
    );
...
test('A session on CS Lembaga Pendidikan answers from live data, sends a document once, then the text answer', async () => {
  const t = await tenant();
  const profile = (
      await t.api('post', '/ai/data-profiles').send({ profile_type: 'pendidikan', name: 'Pesantren Uji' }).expect(201)
    ).body,
    base = '/ai/data-profiles/' + profile.id;
  await t
    .api('post', base + '/programs')
    .send({ name: 'Tahfidz', description: 'Biaya Rp 1.250.000 per bulan.' })
    .expect(201);
  ...
  calls.length = 0;
  const before = sent.filter(s => s.account === t.id).length;
  ...
  // Dokumen dikirim lebih dulu, sebagai file dengan namanya, lalu jawabannya.
  const [document, answer] = out;
  assert.deepEqual(
    'type' in document.content ? [document.content.type, document.content.filename, document.content.mimetype] : [],
    ['document', 'brosur.pdf', 'application/pdf'],
  );
  assert.deepEqual(document.file, pdf);
  assert.deepEqual('text' in answer.content ? answer.content.text : '', 'Ini brosurnya.');
  // Router dan specialist diberi tahu atas nama siapa mereka berbicara dan cara menyebut orang.
  assert.ok(
    calls
      .find(c => c.role === 'router')!
      .messages[0].content.includes('Anda melayani lembaga pendidikan "Pesantren Uji".'),
  );
  assert.ok(
    calls
      .find(c => c.role === 'program')!
      .messages.some(
        m => m.role === 'system' && m.content.includes('Tools tersedia: get_program, get_dokumen, kirim_dokumen'),
      ),
  );
  const history = await eventually(
    () => chatMessages(t.id, 'psb', customer),
    h => h.messages.filter(m => m.origin === 'ai').length === 2,
  );
  assert.deepEqual(
    history.messages.filter(m => m.origin === 'ai').map(m => [m.type, m.text]),
    [
      ['document', 'brosur.pdf'],
      ['text', 'Ini brosurnya.'],
    ],
  ...
  const readMemory = async () => {
    ...
    ).map((m: AIMessage) => m.content) as string[];
  };
  ...
  const [usage] = await db.execute<any[]>(
    'SELECT profile_type,data_profile_id,status FROM ai_usage WHERE account_id=?',
    [t.id],
  );
  assert.deepEqual(
    usage.map(u => [u.profile_type, u.data_profile_id, u.status]),
    [['pendidikan', profile.id, 'sent']],
  );
  // Bertanya lagi: tool menolak pengiriman ulang, jadi hanya jawaban teks yang terkirim.
  t.send('psb', 'E2', 'Kirim brosurnya lagi dong');
  const again = await eventually(mine, v => v.length === 3);
  assert.deepEqual(
    again.slice(2).map(s => ('text' in s.content ? s.content.text : s.content.type)),
    ['Brosurnya sudah dikirim di atas.'],
  );
  // Uji Coba mencantumkan dokumen yang akan dikirim dan tidak mengirim apa pun.
  ...
  // Pemilik mematikan profil menghentikan AI; data profilnya tetap ada.
  await setProfileEnabled(owner, 'pendidikan', false);
  t.send('psb', 'E3', 'Halo');
...
test('Education tools answer from the data they are given and limit what reaches the model', () => {
  const data = {
    ...
    jadwal: 'Tes Sabtu',
    programs: [
      { name: 'Tahfidz Putra', description: 'Biaya A ' + 'x'.repeat(300) },
      { name: 'MA', description: '' },
    ],
    contacts: [{ bagian: 'Keuangan', kontak: '0812', deskripsi: 'SPP' }],
    documents: [{ id: 'd1', filename: 'brosur.pdf', media_type: 'document' as const, description: 'Brosur' }],
  };
  assert.deepEqual(eduTool('get_program', 'tahfidz putra', data), {
    program: [{ nama: 'Tahfidz Putra', deskripsi: data.programs[0].description }],
  });
  const list = eduTool('get_program', '', data) as any;
  assert.equal(list.daftar_program[0].ringkasan.length, 160);
  assert.equal(list.daftar_program[1].nama, 'MA');
  assert.deepEqual((eduTool('get_jadwal', '', data) as any).jadwal, 'Tes Sabtu');
  assert.deepEqual(eduTool('get_dokumen', '', data, ['brosur.pdf']), {
    dokumen: [{ nama_file: 'brosur.pdf', jenis: 'dokumen', deskripsi: 'Brosur', sudah_dikirim: true }],
  });
  assert.deepEqual(eduTool('kirim_dokumen', 'brosur.pdf', data), {
    available: true,
    nama_file: 'brosur.pdf',
    document_id: 'd1',
    jenis: 'dokumen',
  });
...
test('The education pipeline has its own workflow and AI Studio simulation', async () => {
  ...
  assert.deepEqual(
    Object.keys(state.draft.nodes).sort(),
    ['context', 'jadwal', 'kontak', 'lainnya', 'pembuka', 'penutup', 'profil_lembaga', 'program', 'router'].sort(),
  );
  assert.deepEqual(state.allowedTools.kontak, ['get_kontak']);
  assert.deepEqual((state.routerSchema.properties.sub_agent as { enum: string[] }).enum, [
    'pembuka',
    'profil_lembaga',
    'program',
    'jadwal',
  ...
  const edu = {
    ...
    jadwal: '',
    faq: '',
    programs: [{ name: 'Tahfidz', description: 'Biaya' }],
    contacts: [],
    documents: [{ nama_file: 'brosur.pdf', jenis: 'dokumen', deskripsi: 'Brosur' }],
  };
  await studio.run(
    owner,
    { message: 'Minta brosur', revision: saved.revision, profile_type: 'pendidikan', edu, behavior: '' },
    event => events.push(event),
  );
  const output = events.find(e => e.node === 'output');
  assert.deepEqual(
    [output.state, output.output.answer, output.output.documents],
    ['done', 'Ini brosurnya.', ['brosur.pdf']],
  );
  ...
  // Percakapan simulasi yang sama mengingat dokumen yang sudah dikirim.
  events.length = 0;
  await studio.run(
    owner,
    {
  ...
  await assert.rejects(
    studio.run(
      owner,
      {
        ...
        revision: saved.revision,
        profile_type: 'pendidikan',
        edu: { ...edu, documents: [{ nama_file: 'a.pdf' }, { nama_file: 'a.pdf' }] },
        behavior: '',
      },
      () => {},
    ),
