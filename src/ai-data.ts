...
export class AIData implements AITools {
 ...
 async deleteOrder(account:string,session:string,id:string){return transaction(account,async c=>{const [result]=await c.execute<ResultSetHeader>('DELETE FROM ai_orders WHERE account_id=? AND session_id=? AND id=?',[account,session,id]);if(!result.affectedRows)throw missing();return {ok:true};});}
