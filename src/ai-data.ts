...
export interface Product {id:string;name:string;type:'product'|'service';description:string;price:number;stock:number;unit:string;active:boolean}
export function productInput(value:unknown):Product{const p=record(value);if(p.type!=='product'&&p.type!=='service')throw invalid('Jenis produk/layanan tidak valid');if(typeof p.active!=='boolean')throw invalid('Status produk tidak valid');return {id:identifier(p.id),name:text(p.name,150,'Nama'),type:p.type,description:text(p.description??'',500,'Deskripsi',true),price:integer(p.price,1000000000,'Harga'),stock:integer(p.stock,1000000,'Stok/kapasitas'),unit:text(p.unit??'pcs',20,'Satuan',true)||'pcs',active:p.active};}
function productRow(p:RowDataPacket):Product{return productInput({...p,price:Number(p.price),active:Boolean(p.active)});}
export interface OrderInput {items:{product_id:string;quantity:number}[];notes:string}
export function orderInput(value:unknown):OrderInput{const o=record(value);if(Object.keys(o).some(k=>!['items','notes'].includes(k)))throw invalid('Pesanan hanya menerima items dan notes');if(!Array.isArray(o.items)||!o.items.length||o.items.length>20)throw invalid('Isi 1–20 item pesanan');const items=o.items.map(value=>{const item=record(value);if(Object.keys(item).some(k=>!['product_id','quantity'].includes(k)))throw invalid('Item hanya menerima product_id dan quantity');return {product_id:identifier(item.product_id),quantity:integer(item.quantity,1000,'Jumlah',1)};});if(new Set(items.map(i=>i.product_id)).size!==items.length)throw invalid('Gabungkan produk yang sama dalam satu item');return {items,notes:text(o.notes??'',1000,'Catatan',true)};}
export const orderStatuses=['baru','diproses','selesai','dibatalkan'] as const;
export interface Order {id:string;customer:string;items:(OrderInput['items'][number]&{name:string;price:number})[];total:number;status:string;notes:string}
...
export class AIData implements AITools {
 ...
 async products(account:string,session:string,query='',activeOnly=false){const [rows]=await db.execute<RowDataPacket[]>('SELECT id,name,type,description,price,stock,unit,active FROM ai_products WHERE account_id=? AND session_id=?'+(activeOnly?' AND active=TRUE':'')+' AND (id=? OR name LIKE ?) ORDER BY id LIMIT '+(activeOnly?'20':'200'),[account,session,query,'%'+query+'%']);return rows.map(productRow);}
 async saveProduct(account:string,session:string,value:unknown){const p=productInput(value);await transaction(account,async c=>{await c.execute('INSERT INTO ai_products(account_id,session_id,id,name,type,description,price,stock,unit,active) VALUES (?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE name=VALUES(name),type=VALUES(type),description=VALUES(description),price=VALUES(price),stock=VALUES(stock),unit=VALUES(unit),active=VALUES(active)',[account,session,p.id,p.name,p.type,p.description,p.price,p.stock,p.unit,p.active]);});return p;}
 ...
 async catalog(scope:ToolContext,query:string):Promise<Product[]>{const config=await source(scope.account,scope.session,'products');if(config.mode==='builtin')return this.products(scope.account,scope.session,query,true);const result=await this.remoteCall(config,'get_products',query,scope);if(!Array.isArray(result.products)||result.products.length>20)throw Error('endpoint_invalid_products');const products=result.products.map(productInput).filter(p=>p.active);if(new Set(products.map(p=>p.id)).size!==products.length)throw Error('endpoint_duplicate_products');return products;}
 private validateRemoteOrder(value:unknown,scope:ToolContext,expectedId?:string):Order|null{if(value===null)return null;const o=record(value);if(o.customer!==scope.customer||(expectedId&&o.id!==expectedId)||!orderStatuses.includes(o.status as any))throw Error('endpoint_order_scope');const id=identifier(o.id),customer=customerNumber(o.customer),notes=text(o.notes??'',1000,'Catatan',true);if(!Array.isArray(o.items)||!o.items.length||o.items.length>20)throw Error('endpoint_invalid_order');const items=o.items.map(value=>{const i=record(value);return {product_id:identifier(i.product_id),quantity:integer(i.quantity,1000,'Jumlah',1),name:text(i.name,150,'Nama'),price:integer(i.price,1000000000,'Harga')};});const total=integer(o.total,20000000000000,'Total');if(total!==items.reduce((sum,i)=>sum+i.quantity*i.price,0))throw Error('endpoint_invalid_total');return {id,customer,items,total,status:String(o.status),notes};}
 async createOrder(scope:ToolContext,input:OrderInput):Promise<Order>{
  ...
  for(const item of input.items){const product=(await this.catalog(scope,item.product_id)).find(p=>p.id===item.product_id);if(!product||product.stock<item.quantity)throw invalid('Produk tidak tersedia atau stok/kapasitas tidak cukup');items.push({...item,name:product.name,price:product.price});}
 ...
 async execute(name:ToolName,query:string,scope:ToolContext):Promise<unknown>{
  ...
  let input:OrderInput;try{input=orderInput(JSON.parse(query));}catch{throw invalid('create_order memerlukan JSON items [{product_id,quantity}] dan notes');}
  if(config.mode==='builtin')return {order:await this.createOrder(scope,input)};
  const priced=[];for(const item of input.items){const product=(await this.catalog(scope,item.product_id)).find(p=>p.id===item.product_id);if(!product||product.stock<item.quantity)throw invalid('Produk tidak tersedia atau stok/kapasitas tidak cukup');priced.push({...item,name:product.name,price:product.price});}
  const result=await this.remoteCall(config,name,{...input,items:priced},scope);const order=this.validateRemoteOrder(result.order,scope);if(!order)throw Error('endpoint_missing_order');if(order.items.length!==input.items.length||input.items.some(i=>!order.items.some(o=>o.product_id===i.product_id&&o.quantity===i.quantity)))throw Error('endpoint_order_items');return {order};
 }
}
