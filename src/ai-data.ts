...
export interface Product {id:string;name:string;type:'product'|'service';description:string;price:number;stock:number;active:boolean}
export function productInput(value:unknown):Product{const p=record(value);if(p.type!=='product'&&p.type!=='service')throw invalid('Jenis produk/layanan tidak valid');if(typeof p.active!=='boolean')throw invalid('Status produk tidak valid');return {id:identifier(p.id),name:text(p.name,150,'Nama'),type:p.type,description:text(p.description??'',500,'Deskripsi',true),price:integer(p.price,1000000000,'Harga'),stock:integer(p.stock,1000000,'Stok/kapasitas'),active:p.active};}
