...
// Every write also re-checks that the data profile belongs to the account, so an id from another tenant never matches.
async function transaction<T>(account:string,fn:(c:PoolConnection)=>Promise<T>,profile?:string){const c=await db.getConnection();try{await c.beginTransaction();const [rows]=await c.execute<RowDataPacket[]>('SELECT suspended FROM accounts WHERE id=? FOR UPDATE',[account]);if(!rows[0]||rows[0].suspended)throw new ApiError(403,'account_unavailable','Akun tidak tersedia');if(profile!==undefined){const [owned]=await c.execute<RowDataPacket[]>('SELECT id FROM ai_data_profiles WHERE id=? AND account_id=?',[profile,account]);if(!owned[0])throw new ApiError(404,'data_profile_not_found','Data profil tidak ditemukan');}const result=await fn(c);await c.commit();return result;}catch(error){await c.rollback();throw error;}finally{c.release();}}
...
// Products, orders and their sources belong to a data profile (ai_data_profiles), shared by every session it is attached to.
export async function source(account:string,profile:string,kind:SourceKind,c=db):Promise<DataSource>{const [rows]=await c.execute<RowDataPacket[]>('SELECT mode,endpoint,secret FROM ai_data_sources WHERE account_id=? AND data_profile_id=? AND kind=?',[account,profile,kind]);return rows[0]?{mode:rows[0].mode,endpoint:rows[0].endpoint,secret:rows[0].secret}:{...builtinSource};}
export async function publicSources(account:string,profile:string){const products=await source(account,profile,'products'),orders=await source(account,profile,'orders');const safe=({secret,...s}:DataSource)=>({...s,has_token:Boolean(secret)});return {products_source:safe(products),orders_source:safe(orders)};}
...
export async function saveSource(c:PoolConnection,account:string,profile:string,kind:SourceKind,input:Awaited<ReturnType<typeof sourceInput>>){const [rows]=await c.execute<RowDataPacket[]>('SELECT endpoint,secret FROM ai_data_sources WHERE account_id=? AND data_profile_id=? AND kind=?',[account,profile,kind]);const secret=input.clear_token||input.mode==='builtin'?'':input.token?encrypt(input.token):rows[0]?.endpoint===input.endpoint?rows[0].secret:'';await c.execute('INSERT INTO ai_data_sources(account_id,data_profile_id,kind,mode,endpoint,secret) VALUES (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE mode=VALUES(mode),endpoint=VALUES(endpoint),secret=VALUES(secret)',[account,profile,kind,input.mode,input.endpoint,secret]);}
...
export class AIData implements AITools {
 ...
 async products(account:string,profile:string,query='',activeOnly=false){const [rows]=await db.execute<RowDataPacket[]>('SELECT name,type,description,price,stock,active,image_id FROM ai_products WHERE account_id=? AND data_profile_id=?'+(activeOnly?' AND active=TRUE':'')+' AND name LIKE ? ORDER BY name LIMIT '+(activeOnly?'20':'200'),[account,profile,'%'+query+'%']);return rows.map(productRow);}
 async saveProduct(account:string,profile:string,previousName:string,value:unknown){const p=productInput(value);return transaction(account,async c=>{
  // A photo is usable only by the data profile it was uploaded to.
  if(p.image_id){const [image]=await c.execute<RowDataPacket[]>('SELECT id FROM ai_product_images WHERE id=? AND account_id=? AND data_profile_id=?',[p.image_id,account,profile]);if(!image[0])throw invalid('Foto produk tidak ditemukan');}
  if(previousName&&previousName!==p.name)await c.execute('DELETE FROM ai_products WHERE account_id=? AND data_profile_id=? AND name=?',[account,profile,previousName]);
  const [previous]=await c.execute<RowDataPacket[]>('SELECT image_id FROM ai_products WHERE account_id=? AND data_profile_id=? AND name=?',[account,profile,previousName||p.name]);
  await c.execute('INSERT INTO ai_products(account_id,data_profile_id,name,type,description,price,stock,active,image_id) VALUES (?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE type=VALUES(type),description=VALUES(description),price=VALUES(price),stock=VALUES(stock),active=VALUES(active),image_id=VALUES(image_id)',[account,profile,p.name,p.type,p.description,p.price,p.stock,p.active,p.image_id]);
  return {product:p,replacedImageId:previous[0]?.image_id&&previous[0].image_id!==p.image_id?String(previous[0].image_id):null};
 },profile);}
 async orders(account:string,profile:string){const [rows]=await db.execute<RowDataPacket[]>('SELECT * FROM ai_orders WHERE account_id=? AND data_profile_id=? ORDER BY created_at DESC,id DESC LIMIT 200',[account,profile]);return rows.map(row=>({...orderRow(row),session_id:row.session_id??null,created_at:row.created_at}));}
 async order(account:string,profile:string,id:string,customer?:string){const [rows]=await db.execute<RowDataPacket[]>('SELECT * FROM ai_orders WHERE account_id=? AND data_profile_id=? AND id=?'+(customer?' AND customer=?':''),[account,profile,id,...(customer?[customer]:[])]);return rows[0]?orderRow(rows[0]):null;}
 async updateOrder(account:string,profile:string,id:string,value:unknown){const o=record(value);if(!orderStatuses.includes(o.status as any))throw invalid('Status pesanan tidak valid');const notes=text(o.notes??'',1000,'Catatan',true);return transaction(account,async c=>{const [rows]=await c.execute<RowDataPacket[]>('SELECT id FROM ai_orders WHERE account_id=? AND data_profile_id=? AND id=? FOR UPDATE',[account,profile,id]);if(!rows[0])throw missing();await c.execute('UPDATE ai_orders SET status=?,notes=? WHERE account_id=? AND data_profile_id=? AND id=?',[String(o.status),notes,account,profile,id]);return {ok:true};});}
 // Orders never reserve stock, so removing one needs no stock adjustment.
 async deleteOrder(account:string,profile:string,id:string){return transaction(account,async c=>{const [result]=await c.execute<ResultSetHeader>('DELETE FROM ai_orders WHERE account_id=? AND data_profile_id=? AND id=?',[account,profile,id]);if(!result.affectedRows)throw missing();return {ok:true};});}
 private async remoteCall(config:DataSource,name:ToolName,query:unknown,scope:ToolContext){return record(await this.remote(config,{action:name,query,context:{account_id:scope.account,data_profile_id:scope.profile,session_id:scope.session||null,customer:scope.customer,request_id:scope.requestId}},digest(JSON.stringify([scope.account,scope.session,scope.customer,scope.requestId,name,name==='create_order'?'':query]))));}
 async catalog(scope:ToolContext,query:string):Promise<Product[]>{const config=await source(scope.account,scope.profile,'products');if(config.mode==='builtin')return this.products(scope.account,scope.profile,query,true);const result=await this.remoteCall(config,'get_products',query,scope);if(!Array.isArray(result.products)||result.products.length>20)throw Error('endpoint_invalid_products');const products=result.products.map(productInput).filter(p=>p.active);if(new Set(products.map(p=>p.name)).size!==products.length)throw Error('endpoint_duplicate_products');return products;}
 ...
 async createOrder(scope:ToolContext,input:OrderInput):Promise<Order>{
  ...
  const [old]=await db.execute<RowDataPacket[]>('SELECT * FROM ai_orders WHERE account_id=? AND data_profile_id=? AND request_id=?',[scope.account,scope.profile,scope.requestId]);
  ...
  return transaction(scope.account,async c=>{const [existing]=await c.execute<RowDataPacket[]>('SELECT * FROM ai_orders WHERE account_id=? AND data_profile_id=? AND request_id=?',[scope.account,scope.profile,scope.requestId]);if(existing[0]){if(existing[0].customer!==scope.customer||existing[0].input_hash!==hash)throw new ApiError(409,'idempotency_conflict','ID sudah dipakai');return orderRow(existing[0]);}
   const order:Order={id:'ORD-'+randomUUID(),customer:scope.customer,items,total,status:'pesanan_masuk',notes:input.notes};
   await c.execute('INSERT INTO ai_orders(account_id,data_profile_id,session_id,id,request_id,input_hash,customer,items,total,status,notes) VALUES (?,?,?,?,?,?,?,?,?,?,?)',[scope.account,scope.profile,scope.session||null,order.id,scope.requestId,hash,scope.customer,JSON.stringify(items),total,order.status,order.notes]);return order;
  },scope.profile);
 }
 async execute(name:ToolName,query:string,scope:ToolContext):Promise<unknown>{
  ...
  if(name==='check_order'){const id=identifier(query);return {order:orderForAI(config.mode==='builtin'?await this.order(scope.account,scope.profile,id,scope.customer):this.validateRemoteOrder((await this.remoteCall(config,name,id,scope)).order,scope,id))};}
