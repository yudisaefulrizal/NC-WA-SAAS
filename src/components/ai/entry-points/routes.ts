...
import {aiData,orderInput,customerNumber} from '../domain/profiles/cs/data.js';
...
// The Asisten AI HTTP surface: per-session settings, data profiles and their content (CS products and orders,
// CS Lembaga Pendidikan programs, contacts and documents), fallback tickets and the chat view.
// Registered on the authenticated gateway router: res.locals carries accountId and the account's SessionManager.
export function aiRoutes(router:express.Router,{ai,productImages}:{ai:AIService;productImages:ProductImageStore}){
