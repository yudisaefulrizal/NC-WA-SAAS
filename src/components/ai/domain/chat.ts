...
import {insertIgnoreChatMessages,selectChatMessagesByAccountIdSessionId,selectChatMessagesByAccountIdSessionIdCustomer,selectChatMessagesCustomerByAccountIdSessionIdMessageId,selectConversationsByAccountIdSessionId,updateChatMessagesStatusByAccountIdSessionIdMessageId} from '../data-access/chat-queries.js';
import {insertChatMessages,upsertChatMessages} from '../data-access/chat-queries.js';
...
export async function recordIncoming(account:string,session:string,message:IncomingMessage){
 ...
 await insertIgnoreChatMessages(db,[account,session,message.from,message.messageId,message.type,describe(message.type,message.text).slice(0,8000)]);
...
export async function recordOutgoing(account:string,session:string,input:{customer:string;messageId:string;origin:ChatOrigin;type?:string;text:string},executor:Executor=db){
 ...
 await upsertChatMessages(executor,[account,session,input.customer,input.messageId,input.origin,type,describe(type,input.text).slice(0,8000)]);
...
export async function recordNote(account:string,session:string,customer:string,text:string,executor:Executor=db){
 ...
 await insertChatMessages(executor,[account,session,customer,id,text]);
...
export async function updateStatus(account:string,session:string,messageId:string,status:ChatStatus){
 const [rows]=await selectChatMessagesCustomerByAccountIdSessionIdMessageId(db,[account,session,messageId,status]);
 if(!rows[0])return;
 await updateChatMessagesStatusByAccountIdSessionIdMessageId(db,[status,account,session,messageId,status]);
...
export async function listChats(account:string,session:string){
 const [latest]=await selectChatMessagesByAccountIdSessionId(db,[account,session]);
 const [conversations]=await selectConversationsByAccountIdSessionId(db,[account,session]);
...
export async function chatMessages(account:string,session:string,customer:string,before?:unknown){
 ...
 const [rows]=await selectChatMessagesByAccountIdSessionIdCustomer(db,[account,session,customer,...(cursor?[cursor[0],cursor[0],cursor[1]]:[])],cursor);
