...
import {deleteOrdersByAccountIdDataProfileIdId,deleteProductsByAccountIdDataProfileIdName,insertOrders,lockAccountsSuspendedById,lockOrdersIdByAccountIdDataProfileIdId,selectDataProfilesIdByIdAccountId,selectDataSourcesByAccountIdDataProfileIdKind,selectDataSourcesEndpointSecretByAccountIdDataProfileIdKind,selectOrdersByAccountIdDataProfileId,selectOrdersByAccountIdDataProfileIdId,selectOrdersByAccountIdDataProfileIdRequestId,selectProductImagesIdByIdAccountIdDataProfileId,selectProductsByAccountIdDataProfileId,selectProductsImageIdByAccountIdDataProfileIdName,updateOrdersStatusByAccountIdDataProfileIdId,upsertDataSources,upsertProducts} from '../../../data-access/cs-queries.js';
...
export async function source(account:string,profile:string,kind:SourceKind,c=db):Promise<DataSource>{const [rows]=await selectDataSourcesByAccountIdDataProfileIdKind(c,[account,profile,kind]);return rows[0]?{mode:rows[0].mode,endpoint:rows[0].endpoint,secret:rows[0].secret}:{...builtinSource};}
...
export async function saveSource(c:PoolConnection,account:string,profile:string,kind:SourceKind,input:Awaited<ReturnType<typeof sourceInput>>){const [rows]=await selectDataSourcesEndpointSecretByAccountIdDataProfileIdKind(c,[account,profile,kind]);const secret=input.clear_token||input.mode==='builtin'?'':input.token?encrypt(input.token):rows[0]?.endpoint===input.endpoint?rows[0].secret:'';await upsertDataSources(c,[account,profile,kind,input.mode,input.endpoint,secret]);}
...
export class AIData implements AITools {
 ...
 async products(account:string,profile:string,query='',activeOnly=false){const [rows]=await selectProductsByAccountIdDataProfileId(db,[account,profile,'%'+query+'%'],activeOnly);return rows.map(productRow);}
 async saveProduct(account:string,profile:string,previousName:string,value:unknown){const p=productInput(value);return transaction(account,async c=>{
  ...
  if(p.image_id){const [image]=await selectProductImagesIdByIdAccountIdDataProfileId(c,[p.image_id,account,profile]);if(!image[0])throw invalid('Foto produk tidak ditemukan');}
  if(previousName&&previousName!==p.name)await deleteProductsByAccountIdDataProfileIdName(c,[account,profile,previousName]);
  const [previous]=await selectProductsImageIdByAccountIdDataProfileIdName(c,[account,profile,previousName||p.name]);
  await upsertProducts(c,[account,profile,p.name,p.type,p.description,p.price,p.stock,p.active,p.image_id]);
  return {product:p,replacedImageId:previous[0]?.image_id&&previous[0].image_id!==p.image_id?String(previous[0].image_id):null};
 },profile);}
 async orders(account:string,profile:string){const [rows]=await selectOrdersByAccountIdDataProfileId(db,[account,profile]);return rows.map(row=>({...orderRow(row),session_id:row.session_id??null,created_at:row.created_at}));}
 async order(account:string,profile:string,id:string,customer?:string){const [rows]=await selectOrdersByAccountIdDataProfileIdId(db,[account,profile,id,...(customer?[customer]:[])],customer);return rows[0]?orderRow(rows[0]):null;}
 async updateOrder(account:string,profile:string,id:string,value:unknown){const o=record(value);if(!orderStatuses.includes(o.status as any))throw invalid('Status pesanan tidak valid');const notes=text(o.notes??'',1000,'Catatan',true);return transaction(account,async c=>{const [rows]=await lockOrdersIdByAccountIdDataProfileIdId(c,[account,profile,id]);if(!rows[0])throw missing();await updateOrdersStatusByAccountIdDataProfileIdId(c,[String(o.status),notes,account,profile,id]);return {ok:true};});}
 // Orders never reserve stock, so removing one needs no stock adjustment.
 async deleteOrder(account:string,profile:string,id:string){return transaction(account,async c=>{const [result]=await deleteOrdersByAccountIdDataProfileIdId(c,[account,profile,id]);if(!result.affectedRows)throw missing();return {ok:true};});}
 ...
 async createOrder(scope:ToolContext,input:OrderInput):Promise<Order>{
  ...
  const [old]=await selectOrdersByAccountIdDataProfileIdRequestId(db,[scope.account,scope.profile,scope.requestId]);
  ...
  return transaction(scope.account,async c=>{const [existing]=await selectOrdersByAccountIdDataProfileIdRequestId(c,[scope.account,scope.profile,scope.requestId]);if(existing[0]){if(existing[0].customer!==scope.customer||existing[0].input_hash!==hash)throw new ApiError(409,'idempotency_conflict','ID sudah dipakai');return orderRow(existing[0]);}
   const order:Order={id:'ORD-'+randomUUID(),customer:scope.customer,items,total,status:'pesanan_masuk',notes:input.notes};
   await insertOrders(c,[scope.account,scope.profile,scope.session||null,order.id,scope.requestId,hash,scope.customer,JSON.stringify(items),total,order.status,order.notes]);return order;
  },scope.profile);
 }
