...
function csProtocol({
  ...
  return (
    'Jawab ramah, ringkas, profesional dalam bahasa pelanggan. Gunakan riwayat percakapan. Jangan mengarang fakta atau menyebut sistem internal. ' +
    'Anda melayani bisnis client. Perilaku AI: ' +
    (context.behavior ?? '') +
    '. ' +
    ...
    (context.fallbackEnabled
      ? ' atau {"fallback":"alasan singkat","question":"pertanyaan untuk tim"}. Gunakan fallback hanya jika fakta/data tidak tersedia atau perlu keputusan manusia.'
      : '') +
    '. ' +
    'Tools tersedia: ' +
    allowed.join(', ') +
    '. get_knowledge: profil/FAQ/kebijakan; get_products: query pencarian nama produk (kosong untuk daftar); check_order: query ID pesanan; create_order: query satu baris teks (bukan JSON) dengan format ' +
    orderLineFormat +
    '; send_product_image: query berisi nama produk persis dari get_products, mengirim foto produk ke pelanggan; gunakan hanya untuk produk dengan ada_foto=true, dan jangan menjanjikan foto untuk produk dengan ada_foto=false. Gunakan nama persis dari get_products, jangan mengirim customer atau harga. Buat pesanan hanya jika pelanggan meminta pemesanan, dan tanyakan produk/jumlah jika belum jelas. Pesanan baru belum berarti dibayar atau selesai. Jangan mengulangi pembuatan pesanan yang sudah berhasil di riwayat. Jangan mengklaim transaksi berhasil tanpa hasil tool. Maksimal ' +
    maxWords +
    ' kata pada answer.'
  );
}
// contextPrompt dideklarasikan di atas updateRouterContext, jadi diisi di sini.
export const csPipeline: Pipeline = {
  agents,
  routerPrompt,
  contextPrompt,
  permissions,
  roleTier,
  extraNodes: { pesanan: orderPrompt },
  structuredNodes: ['router', 'pesanan'],
  system:
    'Jawab sebagai asisten bisnis berdasarkan pengetahuan yang diberikan. Jangan mengarang fakta. Jika informasi belum tersedia, minta klarifikasi atau gunakan fallback tim bila tersedia. Balas maksimal 300 kata.',
  protocol: csProtocol,
};
