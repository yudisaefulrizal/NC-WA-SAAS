...
import {deleteProductImagesByAccountIdId,insertProductImages,selectProductImagesIdByAccountIdId} from '../../../data-access/cs-queries.js';
...
export class ProductImageStore {
 ...
 async save(accountId: string, profile: string, filename: string, body: Readable) {
  ...
  try {
   ...
   await insertProductImages(db,[id, accountId, profile, compressed.length]);
 ...
 async get(accountId: string, id: string) {
  const [rows] = await selectProductImagesIdByAccountIdId(db,[accountId, id]);
 ...
 async remove(accountId: string, id: string) {
  ...
  await deleteProductImagesByAccountIdId(db,[accountId, id]);
 }
}
