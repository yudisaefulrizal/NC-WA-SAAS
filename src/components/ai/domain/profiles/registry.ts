...
import {insertAuditEvents,insertIgnoreWorkflow,lockWorkflowRevisionByProfileType,selectDataProfiles,selectDataProfilesDISTINCTProfileTypeByAccountId,selectProfileTypesId,selectWorkflow,selectWorkflowActiveByProfileType,selectWorkflowByProfileType,updateWorkflowActiveByProfileType,updateWorkflowDraftByProfileType,upsertProfileTypes} from '../../data-access/profile-queries.js';
...
export async function enabledProfiles(){const [rows]=await selectProfileTypesId(db);const enabled=new Set(rows.map(row=>String(row.id)));return new Set(Object.keys(profileDefinitions).filter(id=>enabled.has(id)));}
...
export async function clientProfiles(account:string){
 ...
 const [used]=await selectDataProfilesDISTINCTProfileTypeByAccountId(db,[account]);const own=new Set(used.map(row=>String(row.profile_type)));
...
export async function adminProfiles(){
 const enabled=await enabledProfiles();
 const [workflows]=await selectWorkflow(db);
 const [usage]=await selectDataProfiles(db);
...
export async function setProfileEnabled(actor:string,id:unknown,value:unknown){
 ...
 await upsertProfileTypes(db,[definition.id,value]);
 await insertAuditEvents(db,[actor,(value?'ai_profile_enabled:':'ai_profile_disabled:')+definition.id]);
...
export async function workflowState(profile:unknown='cs'){
 ...
 const [rows]=await selectWorkflowByProfileType(db,[definition.id]);const row=rows[0];
...
export async function activeWorkflow(profile:string='cs'){const definition=profileDefinition(profile);const [rows]=await selectWorkflowActiveByProfileType(db,[definition.id]);return rows[0]?.active?parse(definition,rows[0].active):definition.defaultWorkflow();}
export async function changeWorkflow(actor:string,profile:unknown,value:unknown,publish=false){
 ...
 try{
  ...
  await insertIgnoreWorkflow(c,[definition.id,JSON.stringify(definition.defaultWorkflow())]);
  const [rows]=await lockWorkflowRevisionByProfileType(c,[definition.id]);
  ...
  if(publish)await updateWorkflowActiveByProfileType(c,[definition.id]);
  else await updateWorkflowDraftByProfileType(c,[JSON.stringify(draft),definition.id]);
  await insertAuditEvents(c,[actor,(publish?'ai_workflow_published:':'ai_workflow_draft_saved:')+definition.id]);
  await c.commit();
