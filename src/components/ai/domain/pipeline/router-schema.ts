// CS Usaha's specialists; other profile pipelines pass their own list to the functions below.
...
export function validateRouterOutput(route:Record<string,unknown>,input:string,available:readonly string[]=[],names:readonly string[]=routerAgentNames){
 ...
 if(typeof route.sub_agent!=='string'||!names.includes(route.sub_agent)
...
export function routerResponseFormat(names:readonly string[]=routerAgentNames){return {type:'json_schema',json_schema:{name:'router_output',strict:true,schema:routerSchema(names)}};}
