...
import {finishMessageUsage,insertAgentFailures,insertIgnoreFallbacks,insertMessageUsage,insertTraceLog,lockConversationForMessage,lockConversationsMessagesRevisionByAccountIdSessionIdCustomer,lockUsageByAccountId,markFallbackFailed,markUsageInterrupted,selectAssistantsByAccountIdSessionIdProfileType,selectConversationsPausedRevisionByAccountIdSessionIdCustomer,selectFallbacksIdQuestionByAccountIdSessionIdCustomer,selectUsageRequestIdByAccountIdRequestId,updateConversationMemoryAndContext,updateFallbacksConfirmationMessageIdById,updateFallbacksNotificationMessageIdById,updateUsageStatus,updateUsageStatusByAccountId} from '../data-access/runtime-queries.js';
import {creditAIWallet,debitAIWallet,insertIgnoreConversations,insertIgnoreWallets,selectWalletsBalanceByAccountId,shareSettingsMemoryLimit,updateConversationMemory} from '../data-access/shared-queries.js';
...
import {pipelines,transaction,lockAccount,parseMemory} from './shared.js';
import type {AIService} from './service.js';
import * as conversations from './conversations.js';
// The WhatsApp runtime: queues incoming messages per customer, runs the profile's pipeline, bills, sends and remembers.
export function incoming(svc:AIService,account:string,manager:SessionManager,session:string,message:IncomingMessage){
  if(message.isGroup||message.type!=='text'||!message.text.trim()||!/^\d{5,20}$/.test(message.from))return Promise.resolve();
  const key=JSON.stringify([account,session,message.from]);if(svc.queued>=128)return Promise.resolve();svc.queued++;
  const task=(svc.queues.get(key)??Promise.resolve()).then(async()=>{if(!await conversations.handleFallbackReply(svc,account,manager,session,message))await handleMessage(svc,account,manager,session,message);}).catch(()=>{console.error('Pemrosesan AI gagal; periksa riwayat penggunaan.');}).finally(()=>{svc.queued--;if(svc.queues.get(key)===task)svc.queues.delete(key);});svc.queues.set(key,task);return task;
 }
export async function stop(svc:AIService){await Promise.all(svc.queues.values());}
export async function recover(svc:AIService,account?:string){
  ...
  await transaction(async c=>{const [rows]=await lockUsageByAccountId(c,account?[account]:[],account);for(const row of rows){await creditAIWallet(c,[row.reserved,row.account_id]);await markUsageInterrupted(c,[row.account_id,row.request_id]);}
   await updateUsageStatusByAccountId(c,account?[account]:[],account);});
 }
export async function handleMessage(svc:AIService,account:string,manager:SessionManager,session:string,message:IncomingMessage){
  // Runs the pipeline of the profile the session's data profile belongs to, while the owner has it switched on.
  const config=await svc.config();if(!config.secret)return;
  const assistant=await svc.assistant(account,session),type=assistant.data_profile?.profile_type??'',pipeline=pipelines[type];if(!assistant.enabled||!pipeline||!assistant.profile_enabled)return;
  ...
  const prepared=await transaction(async c=>{await lockAccount(c,account);
   const [existing]=await selectUsageRequestIdByAccountIdRequestId(c,[account,id]);if(existing[0])return;
   const [current]=await selectAssistantsByAccountIdSessionIdProfileType(c,[account,session,type]);if(!current[0]?.enabled)return;
   ...
   await insertIgnoreConversations(c,[account,session,message.from]);
   const [conversations]=await lockConversationForMessage(c,[account,session,message.from]);if(conversations[0].paused)return;
   ...
   await insertIgnoreWallets(c,[account]);const [wallet]=await selectWalletsBalanceByAccountId(c,[account]);
   const [pending]=await selectFallbacksIdQuestionByAccountIdSessionIdCustomer(c,[account,session,message.from]);
  ...
  const guard=async()=>{const enabled=await svc.assistant(account,session);const [rows]=await selectConversationsPausedRevisionByAccountIdSessionIdCustomer(db,[account,session,message.from]);
  ...
  const trackedTransport:AITransport=async(selected,messages,maxWords)=>{
   ...
   for(let attempt=0;attempt<3;attempt++){
    ...
    try{const result=await svc.transport(selected,messages,maxWords);entry.status='responded';trace?.({node,state:'responded',output:result,model:selected.model,attempt:attempt+1});return result;}
  ...
  try{const result=await runAgents(trackedTransport,config,prepared.messages,prepared.maxWords,{account,profile:prepared.profileId,session,customer:message.from,requestId:id,knowledge:prepared.knowledge,behavior:prepared.behavior,fallbackEnabled:true,pendingFallbacks:prepared.pendingFallbacks,identity:prepared.identity,sentDocuments:alreadySent},{execute:async(name,query,base)=>{
   ...
   try{let result=await svc.tools.execute(name,query,context);
    ...
    await retryPause(0);const result=await svc.tools.execute(name,query,context);trace?.({node:name,state:'done',output:result,duration_ms:Date.now()-start});return result;
   }
   ...
   await insertAgentFailures(db,[account,session,id,lastNode??null,(lastTraceError??errorCode).slice(0,100),message.text.slice(0,4000),lastModel??null,lastMessages?JSON.stringify(lastMessages):null,lastRawOutput?.slice(0,65000)??null,prepared.routerContext]).catch(()=>{});
  }
  ...
  if(!generationFailed)try{routerContext=await updateRouterContext(trackedTransport,config,message.text,answer,contextHistory,pipeline);}catch(error){
   ...
   const errorCode=error instanceof Error?error.message:'unknown_error';
   await insertAgentFailures(db,[account,session,id,'context',(lastTraceError??errorCode).slice(0,100),message.text.slice(0,4000),lastModel??null,lastMessages?JSON.stringify(lastMessages):null,lastRawOutput?.slice(0,65000)??null,prepared.routerContext]).catch(()=>{});
  }
  ...
  await transaction(async c=>{await lockAccount(c,account,true);await creditAIWallet(c,[prepared.reserved-charged,account]);await finishMessageUsage(c,[generationFailed?'fallback_generated':'generated',outputWords,charged,agent,JSON.stringify(modelCalls),modelCalls.find(call=>call.role===agent)?.model??config.model,account,id]);
   if(fallbackId&&fallback)await insertIgnoreFallbacks(c,[fallbackId,account,session,message.from,prepared.fallbackNumber,agent??'lainnya',fallback.reason,fallback.question,prepared.routerContext,JSON.stringify(prepared.messages),message.messageId]);});
  ...
  try{
   ...
   if(!generationFailed&&!fallback&&pendingImageId){
    await guard();
    const readImage=async(imageId:string)=>{const file=await svc.productImages.get(account,imageId);return {path:file.path,mimetype:file.mimetype,cleanup:async()=>{}};};
   ...
   await guard();const confirmation=await sendBilled(account,manager,session,'text',{to:message.from,text:answer},'ai_'+id,undefined,guard);await recordOutgoing(account,session,{customer:message.from,messageId:confirmation.messageId,origin:'ai',text:answer}).catch(()=>{});if(fallbackId)await updateFallbacksConfirmationMessageIdById(db,[confirmation.messageId,fallbackId]);
  ...
  await transaction(async c=>{await lockAccount(c,account,true);await updateUsageStatus(c,[generationFailed&&status!=='cancelled'?'fallback_'+status:status,account,id]);if(status==='sent'){
   ...
   await updateConversationMemoryAndContext(c,[JSON.stringify(memory),routerContext,account,session,message.from]);}});
  }finally{await stopTyping();}
 }
