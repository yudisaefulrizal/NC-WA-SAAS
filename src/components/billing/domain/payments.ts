...
import {insertAuditEvents,insertPaymentConfig,insertPaymentOrders,lockAccountsIdById,lockPaymentOrdersById,markPaymentDenied,markPaymentUnknown,markStalePaymentChecked,selectOpenPaymentOrder,selectPaymentConfig,selectPaymentConfigById,selectPaymentOrdersByAccountIdId,selectPaymentOrdersById,selectPaymentOrdersId,selectPaymentOrdersIdByAccountId,selectRecentPaymentOrders,shareAiSettingsCreditPrice,sharePlansById,touchPaymentCheckedAt,updatePaymentFromGateway,updatePaymentOrdersActivatedAtById,upsertAiWallets,upsertPaymentSettings} from '../data-access/payments-queries.js';

const base=(environment:string)=>environment==='production'?'https://api.midtrans.com':'https://api.sandbox.midtrans.com';
// Called inside the settlement transaction of a paid order; the referral component records its commission here.
export interface PurchaseCommissions {recordPurchaseCommission(c:PoolConnection,account:string,orderId:string,total:number):Promise<unknown>}
...
export class Payments {
 constructor(private call:Transport=transport,private configId?:string,private referral:PurchaseCommissions=noCommissions){}
 async configuration(){const [rows]=await selectPaymentConfig(db);return {configured:Boolean(rows[0]),...(rows[0]??{}),serverKey:rows[0]?'********':null,notificationUrl:(process.env.APP_ORIGIN??'http://127.0.0.1:8067')+'/payments/midtrans/notification'};}
 async configure(actor:string,body:unknown){
  ...
  try{await c.beginTransaction();await insertPaymentConfig(c,[id,String(input.environment),secret]);await upsertPaymentSettings(c,[id]);await insertAuditEvents(c,[actor]);await c.commit();}catch(e){await c.rollback();throw e;}finally{c.release();}
  return this.configuration();
 }
 private async credential(id=this.configId){
  const [rows]=await selectPaymentConfigById(db,id?[id]:[],id);
 ...
 async list(account:string){const [rows]=await selectRecentPaymentOrders(db,[account]);return rows;}
 async order(account:string,id:string){const [rows]=await selectPaymentOrdersByAccountIdId(db,[account,id]);if(!rows[0])throw new ApiError(404,'order_not_found','Pembayaran tidak ditemukan');return rows[0];}
 protected async aiPrice(c:PoolConnection){const [rows]=await shareAiSettingsCreditPrice(c);return Number(rows[0]?.credit_price??0);}
 async create(account:string,planId:unknown,kind:'whatsapp'|'ai'='whatsapp',units:unknown=1){
  ...
  const [stale]=await selectPaymentOrdersIdByAccountId(db,[account]);
  ...
  try{await c.beginTransaction();await lockAccountsIdById(c,[account]);
   const [pending]=await selectOpenPaymentOrder(c,[account]);
  ...
  try{
   ...
   if(['400','401','402','410'].includes(String(data.status_code)))await markPaymentDenied(db,[id]);
   else await this.apply(id,data,false);
  }catch{await markPaymentUnknown(db,[id]);}
  return this.order(account,id);
 }
 private async internal(id:string){const [rows]=await selectPaymentOrdersById(db,[id]);if(!rows[0])throw new ApiError(404,'order_not_found','Pembayaran tidak ditemukan');return rows[0];}
 async reconcile(id:string){const order=await this.internal(id),config=await this.credential(order.config_id);const data=await this.call(config.environment,config.key,'/v2/'+encodeURIComponent(id)+'/status');
  if(String(data.status_code)==='404'){
   ...
   await markStalePaymentChecked(db,[id]);return;
  }
 ...
 private async apply(id:string,data:Record<string,any>,verified:boolean){
  ...
  try{await c.beginTransaction();await lockAccountsIdById(c,[found.account_id]);const [rows]=await lockPaymentOrdersById(c,[id]);const order=rows[0];
   ...
   if(verified&&data.transaction_status==='settlement'&&String(data.status_code)==='200'&&(data.fraud_status===undefined||data.fraud_status==='accept')){
    if(order.kind==='ai'){await upsertAiWallets(c,[order.account_id,order.credits]);}else await activatePackage(c,order.account_id,id,{id:order.plan_id,credits:order.credits,session_limit:order.session_limit});status='settlement';
    await updatePaymentOrdersActivatedAtById(c,[id]);
   ...
   await updatePaymentFromGateway(c,[status,data.transaction_id,qr,expires,id]);await c.commit();
  }catch(e){await c.rollback();throw e;}finally{c.release();}
 }
 ...
 async sweep(){const [rows]=await selectPaymentOrdersId(db);for(const row of rows){try{await this.reconcile(row.id);}catch{await touchPaymentCheckedAt(db,[row.id]);}}}
}
