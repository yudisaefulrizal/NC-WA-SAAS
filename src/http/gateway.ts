import {sessionRoutes,sessionDetailRoutes} from '../components/whatsapp/index.js';
import {aiRoutes} from '../components/ai/index.js';
import {autoShareRouter} from '../components/auto-share/index.js';
import {createAutoShare} from '../components/auto-share/index.js';
import {record} from '../libraries/validation.js';
import {ai as defaultAI} from '../components/ai/index.js';
import express from 'express';
import {TenantWebhooks} from '../components/whatsapp/index.js';
import {MediaStore} from '../components/whatsapp/index.js';
...
import {SessionStore} from '../components/whatsapp/index.js';
import {baileysConnector} from '../components/whatsapp/index.js';
import {referral as defaultReferral} from '../components/referral/index.js';
import {customerOf,onChatChange,recordIncoming,recordOutgoing,updateStatus} from '../components/ai/index.js';
...
export function createGateway(connector?:(accountId:string,store:SessionStore)=>Connector,root=resolve('auth'),ai=defaultAI,referral=defaultReferral) {
 ...
 async function manager(id:string){
  if(!managers.has(id))managers.set(id,(async()=>{
   ...
   result.onBeforeSend=async()=>{const [accounts]=await accountStatus(id);if(!accounts[0]||accounts[0].suspended){await result.applyLimit(0);throw new ApiError(403,'account_suspended','Akun dinonaktifkan');}await result.applyLimit((await basicWallet(id)).session_limit);};
 ...
 router.use(async(req,res,next)=>{
  ...
  if(key){[rows]=await accountByApiKeyHash(digest(key));}
  else {
 ...
 async function refresh(){for(const [id,pending] of managers){const m=await pending;const [rows]=await accountStatus(id);await m.applyLimit(!rows[0]||rows[0].suspended?0:(await basicWallet(id)).session_limit);await media.get(id)?.prune();}}
