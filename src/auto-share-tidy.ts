...
import {callAI, countWords, creditCost, type AIConfig, type AIMessage, type AITransport} from './ai.js';
import {tierConfig} from './ai-models.js';
...
export async function tidyMessage(account:string,original:string,config:AIConfig,transport:AITransport=callAI,note=''):Promise<TidyOutcome>{
 ...
 try{
  ...
  await c.execute("INSERT INTO ai_usage(account_id,request_id,session_id,customer,status,input_words,input_rate,output_rate,reserved,model,agent) VALUES (?,?,'auto-share','share','generating',?,?,?,?,?,'rapikan')",[account,requestId,inputWords,config.input_rate,config.output_rate,reserved,cheap.model]);
  await c.commit();
 ...
 await db.execute('UPDATE ai_wallets SET balance=balance+? WHERE account_id=?',[reserved-charged,account]).catch(()=>{});
 await db.execute('UPDATE ai_usage SET status=?,output_words=?,charged=? WHERE account_id=? AND request_id=?',[answer?'done':'failed',outputWords,charged,account,requestId]).catch(()=>{});
