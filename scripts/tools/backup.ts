...
// Stop the app first: this lock makes database + storage a consistent offline snapshot.
...
try{
 ...
 // storage/ holds WhatsApp logins and client files; auth/ exists only on an install not yet started on this version.
 for(const dir of ['storage','auth'])await cp(resolve(dir),join(snapshot,dir),{recursive:true}).catch(e=>{if(e.code!=='ENOENT')throw e;});
