...
import type {Update} from '../src/engine/sessions.js';
...
try{
 ...
 await db.execute('INSERT INTO plans VALUES (?,?,10000,200,2,TRUE)',[plan,'<img src=x onerror=alert(1)>']);
 ...
 await mkdir('data/browser-check',{recursive:true});await page.screenshot({path:'data/browser-check/desktop.png',fullPage:true});for(const id of ['uji-pesan','integrasi','paket','dokumentasi','nomor']){await page.locator('.tabs a[href="/dashboard/'+id+'"]').click();await page.locator('#'+id).waitFor();}await page.setViewportSize({width:390,height:844});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await mkdir('data/browser-check',{recursive:true});await page.screenshot({path:'data/browser-check/mobile.png',fullPage:true});
