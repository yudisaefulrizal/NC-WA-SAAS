...
import {activeWorkflow,changeWorkflow,defaultWorkflow,workflowState} from '../src/ai-workflow.js';
...
const [original]=await db.query<any[]>('SELECT * FROM ai_workflow WHERE id=1');
let browser;
try{
 ...
 await changeWorkflow(owner,{revision:(await workflowState()).revision,draft:defaultWorkflow()});
 ...
 await page.locator('#chat-input').fill('Pesan produk');await page.locator('#send').click();await page.locator('[data-node="router"][data-state="running"]').waitFor();
 ...
 assert.equal((await activeWorkflow()).nodes.router.tier,'smart');
 ...
 await db.query('DELETE FROM ai_workflow WHERE id=1');if(original[0]){const r=original[0];await db.execute('INSERT INTO ai_workflow(id,draft,active,revision,active_version,published_revision) VALUES (1,?,?,?,?,?)',[typeof r.draft==='string'?r.draft:JSON.stringify(r.draft),r.active?(typeof r.active==='string'?r.active:JSON.stringify(r.active)):null,r.revision,r.active_version,r.published_revision]);}
